...
# Restart main model (SGLang, ~3 min cold start; graceful restart is the recovery path)
docker compose up -d sglang-qwen38 --force-recreate
# Restart everything
...
```bash 
  curl -s --max-time 180 -X POST http://127.0.0.1:8888/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"radixark-qwen38-27b-nvfp4-dspark-262k-think","messages":[{"role":"user","content":"hi"}],"max_tokens":10,"chat_template_kwargs":{"enable_thinking":false}}'
```
...
```bash 
  curl -s --max-time 180 -X POST http://127.0.0.1:8888/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"radixark-qwen38-27b-nvfp4-dspark-262k-think","messages":[{"role":"user","content":"hi"}],"max_tokens":10}' | jq '{content: .choices[0].message.content, reasoning: (.choices[0].message.reasoning_content // .choices[0].message.reasoning), tokens: .usage.completion_tokens}'
```
...
# SGLang — main model
curl -s http://127.0.0.1:8888/v1/models | jq -r '.data[].id'
```
...
# Main model (SGLang — via litellm route; use time command)
time curl -s -X POST http://127.0.0.1:4000/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"radixark-qwen38-27b-nvfp4-dspark-262k-think","messages":[{"role":"user","content":"Count 1 to 100"}],"max_tokens":300,"chat_template_kwargs":{"enable_thinking":false}}' \
