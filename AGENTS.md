...
### Model Management
- **2026-08-17 migration:** MAIN MODEL is now `radixark-qwen38-27b-nvfp4-dflash2-262k-think` — Qwen3.8-27B (RadixArk NVFP4) served by SGLang on port 8888 (mem-fraction 0.50 + 100g docker caps — the GB10-safe config per hasso5703; >0.50 risks hard freeze from untracked flashinfer/autotuner allocations). Exposed via litellm (4000). aeon-qwen36-35b retired. **OOM hardening done 2026-08-17:** old large llama-swap models commented out (qwen3.6 27B/35B, qwen3-coder, ornith-35B, gemma4-31B, 26B DFlash) — do NOT re-enable without re-testing memory co-residency with the SGLang main model. See docs/QWEN38_RESEARCH.md + docs/QWEN38_TESTPLAN.md.
- **SGLang operational rules:** /v1/models answers before engine ready — stall monitor uses a generation probe; graceful `docker compose restart` recovers, SIGKILL/manual `docker kill` does NOT auto-restart (docker semantics). DFlash2 (default) — promoted 2026-08-20 after soak PASS (200/200, +51% solo vs DSpark); beats MTP on code; MTP ~20% faster on fresh codegen (one-flag switch).
- **Main model request rules (radixark-qwen38-27b-nvfp4-dflash2-262k-think):**
  ...
  - Concurrency: **4 running requests max** (scheduler + DFlash2 mamba pool 16 slots = 4/request (corrected sizing per MiaAI 2026-08-18)). 5th+ requests queue. KV pool = 637,649 tokens → 2 full-262K sessions. To raise: `--max-mamba-cache-size` = concurrency x 4 (~6.4 GB more, still inside 0.50 envelope).
