...
- **Main model request rules (radixark-qwen38-27b-nvfp4-dspark-262k-think):**
  ...
  - Concurrency: **4 running requests max** (scheduler + DSpark mamba pool 16 slots = 4/request (corrected sizing per MiaAI 2026-08-18)). 5th+ requests queue. KV pool = 637,649 tokens → 2 full-262K sessions. To raise: `--max-mamba-cache-size` = concurrency x 4 (~6.4 GB more, still inside 0.50 envelope).
